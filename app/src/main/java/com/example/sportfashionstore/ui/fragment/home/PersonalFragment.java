...
public class PersonalFragment extends BaseFragmentViewModel<FragmentProfileBinding, PersonalViewModel> {
    ...
    protected void setupUi() {
        accountSettingAdapter = new UtilityAdapter(
                item -> viewModel.handleAccountSettingFeature(item));
        ...
        viewModel.getAccFeatureList().observe(getViewLifecycleOwner(),
                list -> accountSettingAdapter.setData(list));

        viewModel.getCommonFeatureList().observe(getViewLifecycleOwner(),
                list -> commonSettingAdapter.setData(list));
